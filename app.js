        ...
        html += `
            <div class="customer-card" onclick="openProfile(${index})">
                <div class="customer-header">
                    <span class="customer-id">${escapeHtml(customer.id.substring(0, 8))}</span>
...
<head>
    <meta charset="UTF-8">
    <title>جدول کامل اندازه‌گیری ALFAJR</title>
    <style>
        @page { 
            size: 80mm auto; 
            margin: 0; 
...
<body>
    <div class="label">
        ...
        <div class="customer-info">
            ...
            <div class="customer-id">${escapeHtml(customer.id.substring(0, 8))}</div>
        </div>
        
        <table>
            ${AppConfig.MEASUREMENT_FIELDS.map(field => `
                <tr>
                    <td class="label-cell">${field.replace(/_/g, ' ')}</td>
                    <td class="value-cell">${customer.measurements[field] || '-'} ${field.includes('تعداد') || field.includes('مقدار') ? 'عدد' : 'سانتی‌متر'}</td>
                </tr>
            `).join('')}
        </table>
        ...
        <div class="models-section">
            ...
            <div class="model-item">
                <span class="model-label">ویژگی‌ها:</span>
    ...
    <div class="invoice">
        ...
        <div class="customer-info">
            ...
            <div class="info-row">
                ...
                <span class="info-value">${escapeHtml(customer.id.substring(0, 4))}</span>
            </div>
            <div class="info-row">
        ...
        <table class="details-table">
            ...
            <tr>
                <td class="col-label">مدل آستین</td>
                <td class="col-value">${escapeHtml(customer.models.sleeve || '-')}</td>
            </tr>
            ${customer.models.skirt && customer.models.skirt.length > 0 ? `
            <tr>
                <td class="col-label">مدل دامن</td>
                <td class="col-value">${escapeHtml(customer.models.skirt.join('، '))}</td>
            </tr>
            ` : ''}
            <tr>
        ...
        <div class="price-section">
            ...
            ${customer.paymentReceived ? '<div style="color: green; font-size: 8px; margin-top: 1mm;">✅ پرداخت شده</div>' : '<div style="color: red; font-size: 8px; margin-top: 1mm;">❌ پرداخت نشده</div>'}
        </div>
        ` : ''}
...
window.deleteCurrentCustomer = deleteCustomer;
